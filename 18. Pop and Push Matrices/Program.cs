...
public class Program
{
    ...
    private static bool popPushMatrices = true;
    ...
    private static void RenderCube(float translationY, float rotationAngle)
    {
        openGLApi.Translate(0.0f, translationY, -5.0f); // Push eveything 5 units back into the scene, otherwise we won't see the primitive
        openGLApi.Rotate(rotationAngle, 1, 1, 0); // Rotate everything -60 degrees on the X and Y axis, to show off multiple sides of the cube
    ...
    private static void OnRender(double obj)
    {
        ...
        if (popPushMatrices)
        {
            // Push a new ModelView matrix onto the stack, apply rotation/translation and render
            openGLApi.PushMatrix();
            RenderCube(sceneTranslationY, sceneRotationAngle);
            // Pop the ModelView matrix off of the stack to prevent it affecting future objects
            openGLApi.PopMatrix();
        ...
        {
            // No pushing or popping, rotation/translation applied affects future objects
            RenderCube(sceneTranslationY, sceneRotationAngle);
    ...
    private static void OnKeyDown(IKeyboard keyboard, Key key, int arg3)
    {
        ...
        if (key == Key.P)
        {
            popPushMatrices = !popPushMatrices;
        }
    }
}